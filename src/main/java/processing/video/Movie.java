...
//  }
 ...
 the movie has been playing for 4 seconds, the number 4.0 will be returned.
  ...
  protected final void initGStreamer(PApplet parent, String filename) {
    this.parent = parent;
    ...
    try {      
      ...
      if (playbin == null) {
        ...
        for (String supportedProtocol : supportedProtocols) {
          if (filename.startsWith(supportedProtocol + "://")) {
            try {
              playbin = new PlayBin("Movie Player");
 ...
 be called upon a new frame read event.
  ...
  private void makeBusConnections(Bus bus) {
    bus.connect((Bus.ERROR) (GstObject arg0, int arg1, String arg2) -> {
      System.err.println(arg0 + " : " + arg2);
    });
    bus.connect((Bus.EOS) (GstObject arg0) -> {
      if (repeat) {
        if (0 < rate) {
